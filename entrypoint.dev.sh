...
# 1) 应用目录初始化（首次运行从 /usr/src/www 拷贝）
...
# 2) 日志目录（给 www 可写，避免 cron 无法落日志）
mkdir -p /var/log/cron
chown www:root /var/log/cron
chmod 755 /var/log/cron

# 3) 准备 crontab 目录 & 权限（Alpine/BusyBox 严格要求）
...
SRC="/usr/src/cron/www"         # 成品 crontab
DST="$CRON_DIR/www"

# 4) 若用户 crontab 为空，则安装默认任务（不探测 key、不做模板替换）
if [ ! -s "$DST" ] && [ -f "$SRC" ]; then
  echo "[init] Installing default crontab from $SRC ..."
  cp "$SRC" "$DST"
fi

# 5) 最终确保用户 crontab 权限/行尾合规
if [ -f "$DST" ]; then
...
# 6) 启动 crond（后台），再以前台方式启动 supervisord
/usr/sbin/crond -l 8
